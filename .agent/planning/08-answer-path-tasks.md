...
- **Chế độ `hs` của compose = walkthrough của c8** khi file đó có mặt (`buildWalkthroughPrompt`, `walkthroughSchema`, `validateWalkthrough`; hợp đồng ở `answer/types.ts`: `ClassifyInput` không chứa mã người dùng, `WalkthroughOutput` có `candidates[{heading, assessment, deciding_facts, cite_ids}]`, `conclusion{headings, needs_advance_ruling, missing_facts}`, `tariff_ref`). **Đã nối (nhánh `plan08/wire-walkthrough`):** `answer.service.ts` gọi walkthrough cho mọi lượt `hs` có từ hai nhóm ứng viên còn dòng trong `hs_description`; ít hơn thì rơi về prompt `CHẾ ĐỘ hs` ở §3.1. Keo nối nằm ở `answer/walkthrough.run.ts`: `classifyInput` (id bằng chứng = vị trí nguồn), `verifySections` (chạy `verify()` từng mục, dựng lại cite theo cái còn sống), `policyBlock` (mục CHÍNH SÁCH CHUYÊN NGÀNH do code in, danh mục chưa nạp đọc là "chưa nạp"), `flatten` (tiêu đề mục theo tài liệu mẫu của chủ dự án). `validateWalkthrough` chạy **sau** guards chung; câu nó còn bắt sau một lượt repair thì bị cắt. Độ sâu: `full` khi còn đủ trần 100 s, còn lại `brief`.
...
1. **Che mã** bằng `maskCodes` trong `plan.ts`: port nguyên `HS_TOKEN`, `JOINED_HEADING` và `key` của `dispatch.mjs` ở `a37c663`.
   - Chuẩn hoá NFC trước.
   - Che mã chạy tuyến tính: `plan.spec` đo mỗi dạng (chuỗi khoảng trắng dài, danh sách mã nối dài, dãy 10 số sau "hs", nhiều mã khác nhau…) ở 10.000 và 20.000 ký tự xen kẽ nhau, lấy lần nhanh nhất trong năm lần, và đòi gấp đôi độ dài thì thời gian không quá gấp ba. Không đo ngưỡng ms tuyệt đối, vì jest chạy song song làm nó chập chờn. Trước đó, một chuỗi 2.000 khoảng trắng rồi một chữ số làm `maskCodes` chạy vài giây, vì `GAP` có ba `\s*` liền nhau trong lookbehind. Các callback cũng từng bậc hai: mỗi dãy 9+ số dò lại toàn bộ phần chữ trước nó để tìm "hs", mỗi mã tìm lại trong cả sổ mã, mỗi nhóm trần quét mọi mã trong sổ.
   ...
   - Sau từ khoá, một dãy số liền `\d{4}(\d{2}){0,2}` cũng là mã: "mã hs 848180", "hs 848180 dùng cho van được không" bị che, có trong `userCodes` ở dạng có chấm (`8481.80`), và chốt ở bước 4 bỏ phần nào còn viết nó. Sau `mã` hoặc `mã số`, 6 số liền cũng đọc là phân nhóm, kể cả số hồ sơ ("mã số 123456 của hồ sơ" → vai `premise`, kế hoạch `legal` thành `hs`). "hs" cũng là viết tắt của "hồ sơ", nên "hs số 123456" bị che thừa như vậy, cả "hs số 12" (chương 12) và "hs số 1234" (nhóm 12.34); dãy từ 9 số trở lên sau "hs số" vẫn là `[số]`. Che thừa có chủ ý; chỉ xem lại nếu chủ dự án thấy câu hỏi số hồ sơ bị định tuyến sai khi thử trên Zalo.
   - Dòng 10 số là mã 8 số đầu của nó khi viết có chấm, cách hoặc gạch ngang (`8481.80.9910`, `8481 80 9910`, `8481.80.99.10`, `8481 809910`, `8481.809910`, `8481-80-99-10`, `8481-80-9910`), hoặc viết liền ngay sau `hs`, `mã hs`, `hs code` ("mã hs 8481809910"; không phải `hs số`). Gạch ngang chỉ tách cặp khi đứng giữa mọi cặp ("8481-80-99" là `8481.80.99`), nên ngày ISO (`2005-06-15`), tháng (`01-2026`), khoảng số và khoảng năm ("trọng lượng 1250-1500 kg", "giai đoạn 2006-2010"), số tiêu chuẩn ("ISO 9001-2015"), số điện thoại (`0912-345-678`, `0912-345678`) giữ nguyên. Trần: số điện thoại viết 4+6 có dấu cách ("0912 345678") bị che thừa thành mã. Sau từ khoá, gạch ngang tách không đủ cặp đọc thành nhóm cộng một mã thứ hai giả, vì `-` cũng là từ nối danh sách: "mã hs 8481-809910" cho `userCodes` 84.81 và 8099.10, "mã 8481-8099" cho 84.81 và 80.99; không chữ số nào tới model.
   - Nhóm 4 số đứng trần, không có từ khoá, chỉ bị che khi một mã trong sổ (tin này hoặc phần đã che trước đó) bắt đầu bằng nó (`BARE_HEADING`: "thuộc 3005 hay 3824, mã 3005.10.10"), hoặc khi nó nối sau một mã đã che bằng `,`, `/`, `-`, `hay`, `hay là`, `hoặc`, `và`, `với`, `sang`, có dấu hay không (`JOINED_HEADING`, nhận cả phân nhóm 6 số liền: "mã hs 300510 hay 382490", "mã hs 848180, 848190", "mã hs 300510-382490"). Luật bốn số đầu ở dòng dưới áp cả ở đây: "mã 3005.10.10 và 200000 USD", "mã hs 848180, 202609 lô hàng" chỉ che mã. Số bị luật đó loại, hay một ngày ISO, không cắt danh sách, nên mã viết sau nó vẫn bị che: "mã 3005.10.10, 200000 và 382490" giữ 200000, che 382490; "mã 848180 - 2005-06-15 - 300510" giữ ngày, che 300510. Ngược lại, bốn số sau mã và từ nối mà luật đó không loại thì là nhóm: "mã 3005.10.10 với 1250 cái" che cả 1250 (nhóm 12.50, có dòng `userCodes`), vì dáng mã thắng. Bốn số đứng một mình khác, như "năm 2026", giữ nguyên.
   ...
   - **Giới hạn biết trước (R4 còn hở):** các cách viết sau không bị che, `userCodes` rỗng, nên chốt không có gì để kiểm. Theo dõi riêng.
     ...
     - Cặp đầu tách rời mà không có từ khoá: "84 81 80 99", "84.81.8099", "84-81-80-99", "84.81.80.99.10".
     - Gạch ngang không đứng giữa mọi cặp, có dấu cách hai bên, hoặc gạch dài: "8481-809910", "8481-8482" đứng trần, "8481 - 80 - 99", "mã hs 300510 – 382490".
     - Hai dấu mở liền nhau: "mã hs (‘848180’)".
     - Mục thứ hai của danh sách nằm trong ngoặc đơn, ngoặc kép hay nháy: "mã hs (848180) với (300510)", "mã hs '848180' và '300510'".
     - Dấu "/" ngay sau mã sau từ khoá rơi vào luật số hiệu văn bản, nên cả hai mã không bị che: "mã hs 848180/848190".
     - Danh sách dừng ở mục không phải mã và không phải số mà luật nhóm loại trừ. Đó là: số từ 7 chữ số trở lên, số tiền có chấm, ngày d-m-y hoặc d/m/y, số có "/", mục trong ngoặc hay nháy, hoặc một từ. Mã viết sau mục đó không được che (main cũng vậy): "mã hs 300510, 1500000 và 382490", "mã hs 300510 và 2026 và 20260915 và 382490". Main che năm 2026 thành nhóm giả 20.26 rồi cắt cả tin; nhánh này để năm nguyên, nên mã phía sau có thể tới prompt.
     - Từ nối ngoài danh sách để mã thứ hai hở: "mã hs 300510 hoặc là 382490", "mã hs 300510 và cả 382490", "mã hs 300510; 382490".
...
- **"nguyên văn điều đó":** `scope.article` và `scope.doc` lấy từ `state.legal.citations` → `legalProvision`. Dòng trạng thái của prompt kế hoạch nêu nhãn nguồn (`label`, hoặc `provisionLabel` của state cũ), kèm số hiệu văn bản khi nhãn chưa có nó: "Điều 18 (08/2015/NĐ-CP)".
  - `normalizePlan` giữ `scope.doc` khi số hiệu đó là `documentNumber` của một nguồn trong state (`citedDocs`), và trả đúng cách viết của state. Model viết thiếu cơ quan ban hành ("08/2015") thì vẫn giữ, nếu đúng một văn bản trong state mở đầu bằng số đó ("08/2015/NĐ-CP"); hai văn bản cùng mở đầu như vậy thì bỏ. Chỉ số/năm mới được đọc như vậy: số trần không năm ("08") thì bỏ. Kế hoạch bot gửi lại cũng được đọc như vậy. Số hiệu không có trong chữ người dùng lẫn state thì vẫn bị bỏ: model chỉ nhận ra số hiệu, không tự đặt ra.
