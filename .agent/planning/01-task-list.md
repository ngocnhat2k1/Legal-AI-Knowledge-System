...
> **Kết quả:** chủ dự án **quan sát trực tiếp trên trình duyệt (tab Network)** thấy cổng tra cứu biểu thuế
> customs.gov.vn gọi endpoint `bridge` của research 10
> (`POST https://www.customs.gov.vn/bridge?url=/customs/servletws/bieuthue/APIBieuThue`) và **nhận dữ liệu
> trả về** → dùng `bridge`. Backend IP-thô `http://123.30.210.236:8080/hqcustomsapi/` của research 12 **bỏ qua có
...
> chưa thăm dò. **Còn to-do (không chặn thiết kế):** tái hiện bằng `curl` trần từ mạng công ty và bắt một
> phản hồi mẫu cho một mã HS đã biết để đối chiếu với bộ phận khai báo — hiện mới là quan sát trên tab
> Network, **chưa** tái hiện bằng curl, **chưa** lưu phản hồi mẫu.
...
**Mâu thuẫn, cả hai phía (nay đã giải quyết — xem Kết quả ở trên):**
...
Chúng mô tả **các endpoint khác nhau**, nên cả hai lời tường thuật đều có thể chính xác. **Đã giải quyết 2026-07-18:** chủ dự án quan sát trực tiếp trên trình duyệt (tab Network) thấy cổng gọi `bridge` và nhận dữ liệu về — xác nhận research 10; IP-thô bỏ qua có chủ đích.
...
> **Kết quả: MỘT PHẦN.** Lấy mẫu **21 văn bản đã công bố** (4 Luật, 7 Nghị định, 10 Thông tư; ban hành 2014 → 15/07/2026) qua Server Action vbpl.vn. **Trường `provisionTree` = `null` trên 21/21; `referenceProvisions` = `null` trên mọi tham chiếu** (tham chiếu chỉ ở cấp văn bản; `referenceType` là số nguyên, 8 giá trị quan sát được, không có ánh xạ nhãn). **NHƯNG một Server Action khác — research 04 không thấy — trả về cây điều khoản đầy đủ Chương→Điều→Khoản→Điểm cho 21/21** (5 → 1.300 node, gồm cả văn bản cũ đã di trú).
>
> **Hệ quả cho Giai đoạn 5:** cấu trúc cấp điều khoản để chunk RAG là **có sẵn, máy đọc được**; nhưng **cạnh trích dẫn cấp điều khoản KHÔNG có** — đồ thị đó phải tự dựng. Cây outline chỉ là *cấu trúc*, không phải *nội dung text* (ghép với tab nội dung mới đủ). Tham chiếu treo có thật (control `12898` → HTTP 500). **Bonus:** gateway `https://vbpl-bientap-gateway.moj.gov.vn/api` xác nhận còn sống (`/actuator/health` UP) nhưng route vẫn không ánh xạ được từ client (gọi phía server). Parser + bằng chứng: [research/task-004-vbpl-provisiontree](../../research/task-004-vbpl-provisiontree/README.md). **→ Không đổi quyết định v1 (vẫn Postgres-only, không đụng vbpl); chỉ định hướng schema Giai đoạn 5.**
...
> **Kết quả:** monorepo NestJS (`apps/api`) + Docker Compose (Postgres 17 + pgvector) + Drizzle (migration SQL) + Yarn 4. Migration `0000_enable_pgvector.sql` được áp dụng. **Verify end-to-end 2026-07-18:** clean-clone (không `node_modules`) → `docker compose up --build` → thứ tự `db` healthy → `migrate` exit 0 → `api` boot → `GET /health` = `{"status":"ok","db":"up","pgvector":"0.8.5"}`; idempotent (up lại 2 lần, migration count = 1). Không Qdrant/Neo4j/MinIO/BullMQ.
>
> **Quyết định phát sinh (ghi trong [ADR công cụ repo](../architecture-decisions/2026-07-18-repo-tooling-drizzle-yarn.md)):** Drizzle + postgres.js; Yarn 4 (Corepack, node-modules linker); Node 22 / PG 17 / image `pgvector/pgvector:pg17` / Jest; **db publish host cổng 5433** (tránh đụng Postgres sẵn có trên 5432); TypeScript pin **5.9.x**. Migration TASK-006 chỉ bật pgvector — schema biểu thuế là TASK-007.
...
3. ✅ **`provisionTree` / `referenceProvisions` của vbpl** — TASK-004 **ĐÃ GIẢI QUYẾT 2026-07-18** (một phần): `provisionTree`/`referenceProvisions` = null trên 21/21 văn bản, nhưng cây điều khoản Chương→Điều→Khoản→Điểm CÓ qua một action khác. Cạnh trích dẫn cấp điều khoản phải tự dựng. Xem [research/task-004-vbpl-provisiontree](../../research/task-004-vbpl-provisiontree/README.md).
...
- **API customs.gov.vn: research 10 và 12 MÂU THUẪN → ĐÃ GIẢI QUYẾT (2026-07-18, TASK-002).** Chủ dự án quan sát trực tiếp trên trình duyệt (tab Network) thấy cổng gọi `bridge` (research 10) và nhận dữ liệu về → là endpoint được dùng; backend IP-thô (research 12) bỏ qua có chủ đích. Cả hai mô tả endpoint khác nhau nên đều có thể đúng. API vẫn chỉ là lớp kiểm chứng chéo, không phải nguồn chân lý; tái hiện bằng curl trần, phản hồi mẫu và rate-limit vẫn chưa làm.
