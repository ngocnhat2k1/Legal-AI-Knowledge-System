...
- **Các route gateway của vbpl — VẪN CHƯA ÁNH XẠ (xác nhận lại 2026-07-18, TASK-004).** `https://vbpl-bientap-gateway.moj.gov.vn/api` là base URL hardcode trong bundle client `_next/static/chunks/3600-*.js` (axios, timeout 600s). Còn sống: `/actuator/health` → 200 `{"status":"UP"}`; `/api` → 404 JSON kiểu Spring. **Nhưng route không ánh xạ được từ client** — không có literal `/api/...` nào trong ~4MB JS đã quét; đường dẫn ghép động và gọi **phía server qua Next.js Server Actions**. Đúng như research 04. (~30 phút "một API có tài liệu sẽ xóa bỏ bước headless" hóa ra không khả thi từ phía client; muốn map phải bắt lưu lượng phía server hoặc đoán route.)
- **Ánh xạ `referenceType` int → nhãn** — quan sát được **8 giá trị (1, 3, 4, 7, 8, 9, 10, 12)** qua 21 văn bản (2026-07-18, TASK-004; research 04 chỉ thấy 3, 12); có 27 nhãn nhưng **vẫn không có phép join**.
- **Liệu `provisionTree` / `referenceProvisions` có BAO GIỜ được điền hay không — ĐÃ GIẢI QUYẾT (2026-07-18, TASK-004, một phần).** Lấy mẫu **21 văn bản đã công bố**: trường `provisionTree` = `null` trên 21/21; `referenceProvisions` = `null` trên mọi tham chiếu (tham chiếu chỉ ở **cấp văn bản**). **NHƯNG cây điều khoản Chương→Điều→Khoản→Điểm CÓ qua một Server Action khác** (`94635012466e…`, research 04 không thấy) trên 21/21 (5 → 1.300 node, cả văn bản cũ đã di trú). Vậy: cấu trúc cấp điều khoản để chunk RAG **có sẵn, máy đọc được**; **cạnh trích dẫn cấp điều khoản KHÔNG có — phải tự dựng.** Cây outline là *cấu trúc*, không phải *nội dung text*. Tham chiếu treo có thật (control `12898` → HTTP 500). Xem [research/task-004-vbpl-provisiontree](../../research/task-004-vbpl-provisiontree/README.md).
